...
print_info "Waiting for Mailpit..."
kubectl wait --for=condition=available deployment/mailpit -n ${NAMESPACE} --timeout=120s 2>/dev/null || true
...
# Mailpit is exposed via Kind NodePort mapping (30825 -> localhost:8025).
# Clusters created before that mapping was added need a port-forward fallback.
if ! curl -sf --connect-timeout 2 http://localhost:8025/readyz >/dev/null 2>&1; then
    print_info "Mailpit not reachable on localhost:8025 — starting port-forward fallback..."
    nohup kubectl port-forward -n ${NAMESPACE} svc/mailpit 8025:8025 >/tmp/mailpit-pf.log 2>&1 &
    echo $! >/tmp/mailpit-pf.pid
    sleep 2
    if curl -sf --connect-timeout 2 http://localhost:8025/readyz >/dev/null 2>&1; then
        print_success "Mailpit available at http://localhost:8025 (port-forward PID $(cat /tmp/mailpit-pf.pid))"
        print_info "Recreate the Kind cluster to get direct localhost:8025 access without port-forward"
    else
        print_info "Start Mailpit manually: kubectl port-forward -n ${NAMESPACE} svc/mailpit 8025:8025"
    fi
else
    print_success "Mailpit is reachable at http://localhost:8025"
fi
