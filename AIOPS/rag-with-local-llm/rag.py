import time
import uuid
from pathlib import Path

import chromadb
import httpx
from chromadb.config import Settings as ChromaSettings

from config import settings
from models import QueryResponse, SourceChunk


def new_doc_id() -> str:
    return uuid.uuid4().hex[:12]


def chunk_text(text: str) -> list[str]:
    text = text.strip()
    if not text:
        return []

    size = settings.chunk_size
    overlap = settings.chunk_overlap
    chunks: list[str] = []
    start = 0
    while start < len(text):
        end = start + size
        chunks.append(text[start:end].strip())
        if end >= len(text):
            break
        start = max(end - overlap, start + 1)
    return [c for c in chunks if c]


def create_rag_service(
    *,
    max_attempts: int = 30,
    delay_seconds: float = 1.0,
) -> "RAGService":
    last_error: Exception | None = None
    for attempt in range(1, max_attempts + 1):
        try:
            return RAGService()
        except Exception as exc:
            last_error = exc
            if attempt >= max_attempts:
                break
            time.sleep(delay_seconds)
    raise RuntimeError(
        f"Could not connect to Chroma at {settings.chroma_host}:{settings.chroma_port}"
    ) from last_error


def _model_available(tags_response: dict, model_name: str) -> bool:
    prefix = model_name.split(":")[0]
    for entry in tags_response.get("models") or []:
        name = entry.get("name") or ""
        base = name.split(":")[0]
        if name == model_name or base == prefix:
            return True
    return False


class RAGService:
    def __init__(self) -> None:
        self.http = httpx.Client(base_url=settings.ollama_base_url.rstrip("/"), timeout=120.0)
        self.chroma = chromadb.HttpClient(
            host=settings.chroma_host,
            port=settings.chroma_port,
            settings=ChromaSettings(anonymized_telemetry=False),
        )
        self.collection = self.chroma.get_or_create_collection(
            name=settings.collection_name,
            metadata={"hnsw:space": "cosine"},
        )

    def check_chroma(self) -> tuple[bool, str]:
        try:
            self.collection.count()
            return True, "connected"
        except Exception as exc:
            return False, str(exc)

    def check_llm(self) -> tuple[bool, str]:
        try:
            response = self.http.get("/api/tags")
            response.raise_for_status()
            tags = response.json()
        except Exception as exc:
            return False, f"Ollama unreachable at {settings.ollama_base_url}: {exc}"

        missing: list[str] = []
        if not _model_available(tags, settings.ollama_chat_model):
            missing.append(settings.ollama_chat_model)
        if not _model_available(tags, settings.ollama_embedding_model):
            missing.append(settings.ollama_embedding_model)
        if missing:
            return False, f"Missing Ollama model(s): {', '.join(missing)}"
        return True, f"ollama ({settings.ollama_chat_model}, {settings.ollama_embedding_model})"

    def stats(self) -> tuple[int, int]:
        result = self.collection.get(include=["metadatas"])
        metas = result.get("metadatas") or []
        doc_ids = {m.get("doc_id") for m in metas if m and m.get("doc_id")}
        return len(doc_ids), self.collection.count()

    def list_documents(self) -> list[dict]:
        result = self.collection.get(include=["metadatas"])
        metas = result.get("metadatas") or []
        by_doc: dict[str, dict] = {}
        for meta in metas:
            if not meta:
                continue
            doc_id = meta.get("doc_id")
            if not doc_id:
                continue
            entry = by_doc.setdefault(
                doc_id,
                {"doc_id": doc_id, "source": meta.get("source", "unknown"), "chunks": 0},
            )
            entry["chunks"] += 1
        return sorted(by_doc.values(), key=lambda x: x["source"])

    def _embed(self, texts: list[str]) -> list[list[float]]:
        response = self.http.post(
            "/api/embed",
            json={"model": settings.ollama_embedding_model, "input": texts},
        )
        response.raise_for_status()
        data = response.json()
        embeddings = data.get("embeddings")
        if not embeddings or len(embeddings) != len(texts):
            raise RuntimeError("Unexpected embed response from Ollama")
        return embeddings

    def ingest_text(self, source: str, text: str, doc_id: str | None = None) -> tuple[str, int]:
        doc_id = doc_id or new_doc_id()
        chunks = chunk_text(text)
        if not chunks:
            raise ValueError("No text content to ingest")

        embeddings = self._embed(chunks)
        ids = [f"{doc_id}:{index}" for index in range(len(chunks))]
        metadatas = [
            {"doc_id": doc_id, "source": source, "chunk_index": index}
            for index in range(len(chunks))
        ]
        self.collection.upsert(
            ids=ids,
            embeddings=embeddings,
            documents=chunks,
            metadatas=metadatas,
        )
        return doc_id, len(chunks)

    def ingest_file_bytes(self, filename: str, content: bytes) -> tuple[str, int]:
        text = content.decode("utf-8", errors="replace")
        return self.ingest_text(source=filename, text=text)

    def seed_sample_docs(self, directory: Path) -> int:
        if not directory.is_dir():
            return 0
        seeded = 0
        for path in sorted(directory.glob("*")):
            if not path.is_file():
                continue
            if path.suffix.lower() not in {".txt", ".md", ".markdown"}:
                continue
            text = path.read_text(encoding="utf-8", errors="replace")
            self.ingest_text(source=path.name, text=text)
            seeded += 1
        return seeded

    def query(self, question: str, top_k: int | None = None) -> QueryResponse:
        k = top_k or settings.top_k
        query_embedding = self._embed([question])[0]
        results = self.collection.query(
            query_embeddings=[query_embedding],
            n_results=k,
            include=["documents", "metadatas", "distances"],
        )

        docs = (results.get("documents") or [[]])[0]
        metas = (results.get("metadatas") or [[]])[0]
        distances = (results.get("distances") or [[]])[0]

        sources: list[SourceChunk] = []
        context_parts: list[str] = []
        for index, (doc, meta, dist) in enumerate(zip(docs, metas, distances, strict=False)):
            if not doc or not meta:
                continue
            score = 1.0 - float(dist) if dist is not None else None
            chunk = SourceChunk(
                source=str(meta.get("source", "unknown")),
                doc_id=str(meta.get("doc_id", "")),
                chunk_index=int(meta.get("chunk_index", index)),
                text=doc,
                score=score,
            )
            sources.append(chunk)
            context_parts.append(f"[{chunk.source}]\n{doc}")

        context = "\n\n---\n\n".join(context_parts) if context_parts else "No relevant context found."
        system = (
            "You answer questions using only the provided context. "
            "If the context is insufficient, say you do not know."
        )
        user = f"Context:\n{context}\n\nQuestion: {question}"

        response = self.http.post(
            "/api/chat",
            json={
                "model": settings.ollama_chat_model,
                "messages": [
                    {"role": "system", "content": system},
                    {"role": "user", "content": user},
                ],
                "stream": False,
                "options": {"temperature": 0.2},
            },
        )
        response.raise_for_status()
        answer = (response.json().get("message") or {}).get("content") or ""
        return QueryResponse(answer=answer.strip(), sources=sources)
