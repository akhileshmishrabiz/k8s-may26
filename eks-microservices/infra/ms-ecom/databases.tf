# In-cluster data stores for the ecommerce platform.
#
...
locals {
  cnpg_clusters = var.enable_databases && var.cnpg_enabled ? {
    for db_name in var.cnpg_databases : db_name => {
      db_name   = db_name
...
data "kubernetes_secret_v1" "redis_credentials" {
  count = var.enable_databases && var.redis_enabled ? 1 : 0
...
data "kubernetes_secret_v1" "rabbitmq_credentials" {
  count = var.enable_databases && var.rabbitmq_enabled ? 1 : 0
